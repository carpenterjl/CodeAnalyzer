...
static inline Tag tag_for_name(String name) {
    ...
    // THE ONE CHANGE THIS FILE CARRIES vs upstream tree-sitter-html — see
    // grammars/xaml/README.md, which shows this diff and nothing else.
    //
    // Every tag is a custom tag in XAML, and the three behaviours the HTML table drives
    // are all wrong here:
    //
    //   raw text — HTML's <style> and <script> hold CSS and JavaScript, so the grammar
    //     stops parsing markup at the start tag and resumes at the matching end tag.
    //     WPF's <Style> holds markup, and every declaration written inside one was
    //     therefore discarded, silently, with no error to notice. That is the defect
    //     this variant exists to fix.
    //   void elements — HTML's <br> and <img> never close. XAML has no void elements;
    //     an unclosed tag is a mistake, not a shorthand.
    //   implicit closing — HTML lets <p> be closed by the next block tag. XAML does not,
    //     and its tag names collide with HTML's case-insensitively: <Button>, <Label>,
    //     <Menu> and <Style> all hit rows in the table above.
    //
    // Forcing CUSTOM turns all three off at once, because tag_is_void, tag_can_contain
    // and the SCRIPT/STYLE arms of scan_start_tag_name all test tag.type.
    tag.type = CUSTOM;
#else
